...
export default {
  ...
  methods: {
    ...
    initList() {
      ...
      this.$http.get(urls + "?id=" + this.$route.params.id).then(res => {
         console.log(res)
        if (res.body.code === 200) {
          ...
          this.infoImg =
            res.body.result.c_info_img === null
              ? null
              : res.body.result.c_info_img.split(",");
        ...
        this.$http.post(urls, { userId: userid }).then(res => {
          ...
          if (res.body.code === 200) {
            if (res.body.result.list.length > 0) {
              //如果不为空，发送请求 立即购买接口
              this.$router.push({
                path: "/my/BuyOrder/" + this.$route.params.id
              });
              this.flags = '';
            } else {
              //  弹出添加按钮
